...
class AMP_Core_Block_Handler extends AMP_Base_Embed_Handler {
	...
	public function ampify_video_block( $block_content, $block ) {
		...
		if ( isset( $meta_data['width'] ) ) {
			$block_content = preg_replace( '/(?<=<video\s)/', sprintf( 'width="%d" ', $meta_data['width'] ), $block_content );
		}
		if ( isset( $meta_data['height'] ) ) {
			$block_content = preg_replace( '/(?<=<video\s)/', sprintf( 'height="%d" ', $meta_data['height'] ), $block_content );
		}
